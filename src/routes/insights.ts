...
async function generateJSON(prompt: string, maxTokens = 1200): Promise<string> {
  return withRetry(async () => {
    const response = await (genai as any).models.generateContent({
      model: GEMINI_MODEL,
...
insightsRouter.post("/", async (req: Request, res: Response) => {
  try {
    const { question } = req.body as ApiInsightRequest;
...
${stats.map((s: any) => `  • ${s.category}: ${s.total} total (${s.high} high-severity)`).join("\n")}

─ Top Complaint Hotspots:
${hotspots.map((h: any) => `  • ${h.neighborhood}: ${h.total} complaints (${h.high_severity} high-severity)`).join("\n")}

─ 20 Most Recent Complaints:
${recent.map((c: any) => `  [${c.severity ?? "Unknown"}] ${c.category ?? "Other"} — ${(c.text ?? "").slice(0, 80)} (${c.neighborhood ?? "Unknown"})`).join("\n")}
...
`.trim();
    ...
    const answer =
      (await generateText(contextBlock, question, 400)) ||
...
let insightCache: { data: unknown; generatedAt: number } | null = null;
const CACHE_TTL_MS = 30 * 60 * 1000; // 30 minutes

insightsRouter.get("/auto", async (_req: Request, res: Response) => {
  try {
...
insightsRouter.post("/analyze", async (_req: Request, res: Response) => {
  try {
    ...
    for (const complaint of unanalyzed) {
      try {
        const cat = (complaint as any).categories?.name ?? "Other";
