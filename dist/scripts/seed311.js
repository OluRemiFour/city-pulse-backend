...
function mapCategory(requestType) {
    if (!requestType)
        return 'Other';
...
function mapStatus(rawStatus) {
    ...
    if (lower.includes('closed') || lower.includes('complete'))
        return 'Closed';
    if (lower.includes('progress') || lower.includes('assigned') || lower.includes('work'))
        return 'In Progress';
...
function buildDescription(feature) {
    ...
    if (desc)
        return `${type}: ${desc}`.slice(0, 500);
    if (addr)
        return `${type} reported at ${addr}`.slice(0, 500);
...
async function main() {
    ...
    const rows = features.map((f) => {
        ...
        if (lat && (lat < 30 || lat > 35))
            lat = null;
        if (lng && (lng < -90 || lng > -82))
            lng = null;
        return {
