...
statsRouter.get('/', async (_req, res) => {
    try {
        ...
        if (hotspotsRes.error)
            throw hotspotsRes.error;
        if (recentRes.error)
            throw recentRes.error;
        const byCategory = (categoriesRes.data ?? []);
        ...
        const trendByDay = recentTrend.reduce((acc, r) => {
        ...
        const trendChart = Object.values(trendMap).sort((a, b) => new Date(a.day).getTime() - new Date(b.day).getTime());
